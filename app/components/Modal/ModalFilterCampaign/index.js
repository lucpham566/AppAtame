...
function ModalFilterCampaign({
  ...
  const onSubmitFilter = () => {
    ...
    const checkFilter = campaign => {
      for (var i = 0; i < filterList.length; ++i) {
        if (
          !operator_table[filterList[i].bieuThuc](
            campaign.report[filterList[i].value],
            filterList[i].data,
          )
        ) {
          return false;
    ...
    let dataF = dataProductAds.filter(i => {
      return checkFilter(i);
    });

    setProductAdsList([...dataF]);
    toggleModalFilter();
  };
  ...
  const onReset = () => {
    setData([...initData]);
    toggleModal();
    setProductAdsList([...dataProductAds]);
  };
...
export default ModalFilterCampaign;
